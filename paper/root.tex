...
\lstdefinestyle{corelang}{
  ...
  morekeywords={
  repeat, times, foreach, visit, pick, drop, containing, avoiding, strict
  },%in,
  sensitive=true,
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
...
\title{\LARGE \bf Declarative but Natural Robot Programming using Semantic Parsing 
}


\author{Ivan Gavran$^1$ \quad Brendon Boldt$^2$ \quad Eva Darulova$^1$ \quad Rupak Majumdar$^1$%
\thanks{
$^1$Max Planck Institute for Software Systems, Germany, $^2$Marist College, USA}%
\thanks{Brendon Boldt was supported by a DAAD Internship.}
}
...
\input{coreLanguage.tex}

\input{naturalization.tex}
